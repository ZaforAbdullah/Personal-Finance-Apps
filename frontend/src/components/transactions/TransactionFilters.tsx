...
export default function TransactionFilters({ search, category, sortBy, onSearchChange, onCategoryChange, onSortChange }: TransactionFiltersProps) {
  return (
    <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <span style={{ fontSize: '0.875rem', color: '#696868', whiteSpace: 'nowrap' }}>Category</span>
          <Dropdown value={category} options={CATEGORIES} onChange={(e) => onCategoryChange(e.value)} style={{ minWidth: 160, height: '2.875rem' }} />
        </div>
      </div>
    </div>
  );
